...
def _process_tokens(example: Dict[str, Any], model: "PreTrainedModel" = None, **kwargs) -> Dict:
    ...
    if not kwargs["is_encoder_decoder"]:
        ...
        if not kwargs.get('joint_distribution'):
        ...
        # TODO: for joint distribution p(x,y) won't work in encoder-decoder
...
class KTOFixedTrainer(Trainer):
    ...
    Args:
        ...
        joint_distribution (`bool`, defaults to True):
    ...
    def __init__(
        ...
        joint_distribution: Optional[bool] = True
    ):
        if type(args) == TrainingArguments:
            raise ValueError("Please use `BPOConfig` instead TrainingArguments.")
        ...
        # BPO parameter
        assert args.beta > 0, 'beta should be > 0 for algorithm to work'
        self.beta = args.beta
        self.desirable_weight = args.desirable_weight
        self.undesirable_weight = args.undesirable_weight
        self.loss_type = args.loss_type
        self.aux_loss_enabled = getattr(model.config, "output_router_logits", False)
        self.z = self.estimate_z(train_dataset)
    ...
    def estimate_z(self, dataset):
        labels = torch.tensor(dataset['label'], dtype=torch.int8)
        z = torch.sum( torch.exp(1 / self.beta*labels) ) / len(labels)
        return z
    ...
    def bpo_loss(
        ...
        if policy_chosen_logps.shape[0] != 0 or reference_chosen_logps.shape[0] != 0:
            ...
            rewards = chosen_logratios + torch.log1p(self.z)
            chosen_losses = (self.beta * rewards - 1)**2
            chosen_rewards = rewards.detach()
        else:
        ...
        if policy_rejected_logps.shape[0] != 0 or reference_rejected_logps.shape[0] != 0:
            ...
            rewards = rejected_logratios + torch.log1p(self.z)
            rejected_losses = (self.beta * rewards)**2
            rejected_rewards = rewards.detach()
        else:
            # lists can't be empty -- if they are, then accelerate.gather will hang
    ...
    def get_batch_loss_metrics(
        ...
        losses, chosen_rewards, rejected_rewards = self.bpo_loss(
            policy_chosen_logps,
            policy_rejected_logps,
            reference_chosen_logps,
            reference_rejected_logps,
    ...
    def evaluation_loop(
        ...
        if self.generate_during_eval:
            ...
            target_indicies = [i for i in range(len(random_batch["prompt"]))]
            target_batch = {
